(() => {
  ...
  function render(rows) {
    ...
    body.innerHTML = rows.length
      ? rows.map((row) => `<tr><td class="journey-imei">${escapeHtml(row.imei)}</td><td>${escapeHtml(row.device_details || "—")}</td><td>${escapeHtml(formatDateTime(row.date_received))}</td><td>${escapeHtml(formatDateTime(row.date_completed))}</td><td>${escapeHtml(row.stock_channel)}</td><td>${escapeHtml(row.invoice_number)}</td><td>${escapeHtml(row.supplier_company)}</td><td>${escapeHtml(row.quantity_received)}</td><td>${escapeHtml(row.supplier_code)}</td><td>${escapeHtml(row.battery_health || "—")}</td><td>${escapeHtml(row.supplier_grade)}</td><td>${escapeHtml(row.company_initial_grade)}</td><td>${escapeHtml(row.company_final_grade)}</td><td class="journey-parts">${escapeHtml(row.parts_issued)}</td><td class="journey-money">${escapeHtml(formatMoney(row.parts_cost))}</td><td>${escapeHtml(row.service_done)}</td><td>${escapeHtml(row.technician_name)}</td><td>${escapeHtml(row.box_number)}</td><td>${escapeHtml(formatDateTime(row.export_date))}</td><td>${escapeHtml(row.customer_name)}</td></tr>`).join("")
