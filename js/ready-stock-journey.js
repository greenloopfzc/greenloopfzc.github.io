(() => {
  ...
  function render(rows) {
    ...
    body.innerHTML = rows.length
      ? rows.map((row) => `<tr><td>${escapeHtml(formatDateTime(row.stock_received_at))}</td><td>${escapeHtml(supplierLabel(row.supplier_code, row.stock_batch))}</td><td>${escapeHtml(row.stock_channel)}</td><td>${escapeHtml(row.stock_batch)}</td><td class="journey-imei">${escapeHtml(row.imei)}</td><td>${escapeHtml(row.serial_number)}</td><td>${escapeHtml(row.phone_region)}</td><td>${escapeHtml(row.model)}</td><td>${row.storage_gb ? `${escapeHtml(row.storage_gb)} GB` : "—"}</td><td>${escapeHtml(row.color)}</td><td>${row.initial_battery_health == null ? "—" : `${escapeHtml(row.initial_battery_health)}%`}</td><td>${row.final_battery_health == null ? "—" : `${escapeHtml(row.final_battery_health)}%`}</td><td>${escapeHtml(row.supplier_grade)}</td><td>${escapeHtml(row.company_initial_grade)}</td><td>${escapeHtml(row.company_final_qc_grade)}</td><td class="journey-parts">${escapeHtml(row.parts_used)}</td><td class="journey-money">${escapeHtml(formatMoney(row.parts_cost))}</td><td>${escapeHtml(row.work_done)}</td><td>${escapeHtml(row.worked_by)}</td><td>${escapeHtml(row.step_by_step)}</td><td>${escapeHtml(formatDateTime(row.final_qc_passed_at))}</td></tr>`).join("")
